    ...
    const tkey &key)
{
    ...
    while (true)
    {
        ...
        if (can_take_from_left_brother)
        {
            ...
            search_tree<tkey, tvalue>::swap(std::move(parent->keys_and_values[position - 1]), std::move(left_brots[left_brother->viher->keys_and_valuertual_size - 1]));
